...
class TestPIInvoiceExport(BaseTestCaseWithTempDir):
    ...
    def test_pi_invoice_pdf_generation(self):
        pi_invoice = PIInvoice(
            name=self.output_dir, invoice_month=self.invoice_month, data=self.df
        )
        pi_invoice.process()
        pi_invoice.export()
        ...
        expected_filename = "Test University_Jane Doe_2024-01.pdf"
        self.assertIn(expected_filename, pdf_files)
